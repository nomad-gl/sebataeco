...
import { Globe, Eye, EyeOff, Loader2, ShieldCheck, CheckCircle2, ExternalLink, ArrowRight } from "lucide-react";
...
export default function LocalLogin() {
  ...
  useEffect(() => {
    if (!authLoading && isAuthenticated && !showDestinationChoice) {
      window.location.href = returnPath;
    }
  }, [authLoading, isAuthenticated, returnPath, showDestinationChoice]);
        ...
        <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl shadow-2xl overflow-hidden">
          <Tabs defaultValue="login" className="w-full">
            ...
            <TabsContent value="login" className="p-6">
              {/* Destination choice — shown after successful login */}
              {showDestinationChoice && (
                <div className="space-y-5 text-center">
                  ...
                  <div className="flex flex-col gap-3">
                    <Button
                      className="w-full bg-red-600 hover:bg-red-700 text-white font-semibold h-11 gap-2"
                      onClick={() => { window.location.href = returnPath; }}
                    >
                      <ArrowRight className="w-4 h-4" />
                      {t("signin_continue_seba")}
                    </Button>
                    <Button
                      variant="outline"
                      className="w-full border-white/25 text-white hover:bg-white/10 hover:text-white h-11 gap-2"
                      onClick={() => { window.open("https://sebasnap.com", "_blank", "noopener,noreferrer"); }}
                    >
                      <ExternalLink className="w-4 h-4" />
                      {t("signin_go_sebasnap")}
                    </Button>
                  </div>
                </div>
              )}
