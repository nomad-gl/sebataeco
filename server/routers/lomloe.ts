...
Return ONLY a valid JSON object (no markdown, no code fences) with exactly these fields:
{
  ...
  /**
   * Save an AINA creation as a situació
   * Allows AINA chat creations to be saved to My Situacions with print functionality
   */
  saveAinaCreationAsSituacio: protectedProcedure
    .input(
      z.object({
        title: z.string().min(1).max(256),
        topic: z.string().min(1).max(256),
    ...
    .mutation(async ({ ctx, input }) => {
      const db = getDb();
      const competencyStr = input.competencies.join(",");
      const resultJsonStr = JSON.stringify(input.resultJson);
      ...
      return { id: result.insertId, title: input.title };
    }),
